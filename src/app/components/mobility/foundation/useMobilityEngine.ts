...
export function useMobilityEngine() {
  ...
  return {
    metrics: {
      ...
      congestionIndex: data?.metrics?.congestionIndex || null,
      predictedDelay: data?.metrics?.predictedDelay || 'Unknown',
      networkAvailability: snapshots.length > 0 ? 100 : 0,
      fleetReadiness: data?.metrics?.fleetReadiness || null,
      signalHealth: data?.metrics?.signalHealth || null,
      activeRoutes: data?.metrics?.activeRoutes || null,
      fleetStatus: data?.metrics?.fleetStatus || null,
      transitCapacity: data?.metrics?.transitCapacity || null,
      carbonOffset: data?.metrics?.carbonOffset || null,
    },
