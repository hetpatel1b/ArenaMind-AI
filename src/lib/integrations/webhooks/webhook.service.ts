...
export class WebhookService {
   ...
   */
  async dispatch(url: string, payload: SafeAny, secret?: string): Promise<void> {
    try {
      const headers: Record<string, string> = { 'Content-Type': 'application/json' };

      if (secret) {
        const signature = this.generateSignature(payload as string | object, secret);
      ...
      errorTracker.captureException(errInstance, undefined, { webhookUrl: url });
      throw new Error(`Webhook dispatch failed: ${errInstance.message}`);
    }
  }
